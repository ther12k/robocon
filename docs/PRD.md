...
| **R1 SimCore separation** | Simulation core free of DOM imports; tick-based command queue; stable entity IDs + snapshot format; versioned schemas for arena/robot/ruleset/profile/replay; competition-ruleset vs simulation-profile split; replay metadata + state hashes; Vite base-path correctness; arena capability model (elevation, compound colliders, trigger volumes, mechanisms) | Deterministic replay round-trips in CI; schema version negotiation tested |
...
| **M5 Polish & Beta** | Replay viewer, minimap, telemetry graphs, i18n, sharing links | Beta to pilot teams (3–5 universities); feedback round |
